...
int main(int argc, char** argv)
{
  ...
  double param_val = strtod(argv[1], NULL);
  double test = static_cast<double>(param_val);
  if (param_val <= 2.0 && param_val >= -2.0)
  {
    RCLCPP_INFO_STREAM(LOGGER, "Param value = " << param_val);
  }
  else
    return -1;
  ...
  auto callback = [captured_pub, captured_node, test]() -> void {
